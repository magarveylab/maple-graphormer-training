...
class ChemotypeTrainingDataset(Dataset):
...
class TanimotoTrainingDataset(Dataset):
    ...
    def prepare_data(
        self, datapoints: List[dict], subset: Optional[int] = None
    ):
        # load labels
        self.label_dict = {}
        self.sorted_datapoints = {}
        for p in datapoints:
            key = (p["query_peak_id"], p["subject_peak_id"])
            self.label_dict[key] = {
                "b1": p["tanimoto_bin_1"],
                "b2": p["tanimoto_bin_2"],
                "b3": p["tanimoto_bin_3"],
            ...
            cls_bin = p["tanimoto_bin_2"]
            if cls_bin not in self.sorted_datapoints:
                self.sorted_datapoints[cls_bin] = []
            self.sorted_datapoints[cls_bin].append(key)
        # take subset
        if subset:
            for cls_bin in self.sorted_datapoints:
                n = len(self.sorted_datapoints[cls_bin])
        ...
        all_peak_ids = set()
        for cls_bin in self.sorted_datapoints:
            for pair in self.sorted_datapoints[cls_bin]:
                all_peak_ids.update(pair)
        # load graphs
        self.tensor_cache = {}
        for peak_id in tqdm(all_peak_ids):
            graph_fp = f"{self.root}/{peak_id}.pkl"
            G = pickle.load(open(graph_fp, "rb"))
            self.tensor_cache[peak_id] = G.get_tensor_data(
                node_vocab=self.node_vocab,
                edge_vocab=self.edge_vocab,
    ...
    def get(self, input_idx):
        ...
        for name in ["b1", "b2", "b3", "b4", "b5"]:
            label = self.label_dict[(p1, p2)][name]
            setattr(
                common_y,
                f"{name}___a___b",
        ...
        response = MultiInputData(
            graphs={
                "a": self.tensor_cache[p1].graphs["a"],
                "b": self.tensor_cache[p2].graphs["a"],
            },
            common_y=common_y,
        )
        return response
